...
- **A light element's styles are `@scope`, and `@scope` reached the last engine
  ...
  it does not know, and everything inside it. So an element in an older browser
  renders with its markup and none of its own styles, and nothing errors anywhere.
  The same rules in the app's stylesheet reach a light element, since it sits in
  the page's own DOM, and that is the answer for a site that has to go back
  further. `/docs/browsers` says this to a reader, with the dates for the other
  four platform features.
